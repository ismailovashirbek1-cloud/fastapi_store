...
from shop_app.database.schema import UserProfileInputSchema, UserProfileOutSchema, UserLoginShema
from sqlalchemy.orm import Session
from passlib.context import CryptContext
from fastapi.security import OAuth2PasswordBearer
from shop_app.config import (SECRET_KEY, ALGORITHM,
...
async def login(user: UserLoginShema, db: Session = Depends(get_db)):
   user_db = db.query(UserProfile).filter(UserProfile.username==user.username).first()
